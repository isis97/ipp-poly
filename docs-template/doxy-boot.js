window.doxySetup = (function() {
  ...
  $("tbody>*:contains('typedef')").html(function(_, html) {
      return html.replace(/(typedef)/g, '<span class="keyword-typedef">$1</span>');
  });
  /*$("tbody>*:contains('struct')").html(function(_, html) {
      return html.replace(/(struct)/g, '<span class="keyword-struct">$1</span>');
  });*/
  $("tbody>*:contains('#define')").html(function(_, html) {
      return html.replace(/(#define)/g, '<span class="keyword-define">$1</span>');
  });
  $("td.paramdir:contains('in')").addClass("paramdir-in");
